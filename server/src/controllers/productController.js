...
exports.getAllProducts = async (req, res) => {
    try {
        const { category_id, brand_id, search, sort, page = 1, limit = 12, featured } = req.query;
        let query = `
        ...
        if (sort === 'price_asc') query += ' ORDER BY p.selling_price ASC';
        else if (sort === 'price_desc') query += ' ORDER BY p.selling_price DESC';
...
exports.createProduct = async (req, res) => {
    try {
        const { product_name, category_id, brand_id, unit_id, mrp, selling_price, offer_price, tax_percent, hsn_code, description, is_featured } = req.body;
        const slug = product_name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        const [result] = await db.query(
            `INSERT INTO products (product_name, slug, category_id, brand_id, unit_id, mrp, selling_price, offer_price, tax_percent, hsn_code, description, is_featured)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [product_name, slug, category_id, brand_id, unit_id, mrp, selling_price, offer_price || null, tax_percent || 0, hsn_code, description, is_featured ? 1 : 0]
        );
...
exports.updateProduct = async (req, res) => {
    try {
        const { product_name, category_id, brand_id, unit_id, mrp, selling_price, offer_price, tax_percent, hsn_code, description, is_featured, is_active } = req.body;
        const slug = product_name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        await db.query(
            `UPDATE products SET product_name=?, slug=?, category_id=?, brand_id=?, unit_id=?, mrp=?, selling_price=?, offer_price=?, tax_percent=?, hsn_code=?, description=?, is_featured=?, is_active=? WHERE product_id=?`,
            [product_name, slug, category_id, brand_id, unit_id, mrp, selling_price, offer_price || null, tax_percent, hsn_code, description, is_featured ? 1 : 0, is_active ? 1 : 0, req.params.id]
        );
