  ...
  return (
    <div style={{ padding: '30px 50px', background: '#f8f9fa', minHeight: '80vh' }}>
      ...
      <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        <Row gutter={40}>
          <Col xs={24} md={10}>
            <div style={{ height: 350, background: '#fafafa', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '8rem', marginBottom: 16 }}>
              {categoryEmojis[product.category_name] || '📦'}
            </div>
          </Col>
          <Col xs={24} md={14}>
            <Tag color="blue" className="mb-2">{product.category_name}</Tag>
            <Title level={2} style={{ marginBottom: 4 }}>{product.product_name}</Title>
            {product.brand_name && <Text type="secondary" style={{ fontSize: '1rem' }}>by {product.brand_name}</Text>}

            <Divider />

            <div className="d-flex align-items-baseline gap-3 mb-2">
              <Text strong style={{ fontSize: '2rem', color: '#1890ff' }}>₹{displayPrice}</Text>
              {discount > 0 && (
                <>
                  <Text delete type="secondary" style={{ fontSize: '1.2rem' }}>₹{product.mrp}</Text>
                  <Tag color="red" style={{ fontSize: '0.9rem', padding: '2px 12px' }}>{discount}% OFF</Tag>
                </>
              )}
            </div>
            {product.tax_percent > 0 && <Text type="secondary" style={{ fontSize: '0.85rem' }}>Inclusive of {product.tax_percent}% tax</Text>}

            <div className="mt-3 mb-3">
              {product.stock_qty > 0 ? (
                <Tag icon={<CheckCircleOutlined />} color="success" style={{ padding: '4px 12px' }}>In Stock ({product.stock_qty} available)</Tag>
              ) : (
                <Tag color="error" style={{ padding: '4px 12px' }}>Out of Stock</Tag>
              )}
            </div>
            ...
            <div className="d-flex gap-3 mb-4">
              <Button type="primary" size="large" icon={<ShoppingCartOutlined />} shape="round" style={{ height: 48, paddingInline: 32 }} disabled={product.stock_qty === 0}>
      ...
      {product.description && (
        <Card className="mt-4" style={{ borderRadius: 16, border: 'none' }}>
          <Tabs items={[
            ...
            { key: 'info', label: 'Product Info', children: (
              <div>
