...
const AdminOrders = () => {
  ...
  const statusStepMap: Record<string, number> = { 'Placed': 0, 'Accepted': 1, 'Packed': 2, 'Out for Delivery': 3, 'Delivered': 4 };

  const columns = [
    { title: 'Order #', dataIndex: 'order_number', render: (v: string) => <Text strong style={{ color: '#1890ff' }}>{v}</Text> },
    ...
    { title: 'Payment', dataIndex: 'payment_method', render: (v: string) => <Tag>{v || 'N/A'}</Tag> },
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <Space>
          ...
          <Select value={statusFilter} onChange={setStatusFilter} style={{ width: 150 }} options={[
            { value: 'all', label: 'All Status' }, { value: 'Placed', label: 'Placed' }, { value: 'Packed', label: 'Packed' },
            { value: 'Out for Delivery', label: 'Out for Delivery' }, { value: 'Delivered', label: 'Delivered' }, { value: 'Cancelled', label: 'Cancelled' },
          ]} />
        </Space>
      </div>
      <Card style={{ borderRadius: 12 }} bodyStyle={{ padding: 0 }}>
        <Table columns={columns} dataSource={orders.map((o: any, i: number) => ({ ...o, key: i }))} loading={loading} pagination={{ pageSize: 10 }} />
      </Card>
        ...
        {detailOrder && (
          <div>
            ...
            <Steps current={statusStepMap[detailOrder.order_status] ?? 0} size="small" items={[
              { title: 'Placed' }, { title: 'Accepted' }, { title: 'Packed' }, { title: 'Out for Delivery' }, { title: 'Delivered' },
            ]} />
            <Divider />
            ...
            <Space className="w-100" style={{ justifyContent: 'flex-end' }}>
              <Select defaultValue={detailOrder.order_status} style={{ width: 180 }} options={Object.keys(statusColors).map(s => ({ value: s, label: s }))}
