...
const AdminDashboard = () => {
  ...
  return <div>
      ...
      <Row gutter={[16, 16]} className="mb-4">
        {stats.map((stat, i) => <Col xs={24} sm={12} md={6} key={i}>
            <Card style={{
          borderRadius: 12,
          borderLeft: `4px solid ${stat.color}`
        }}>
              <Statistic title={stat.title} value={stat.value} prefix={stat.prefix} valueStyle={{
            color: stat.color
          }} />
            </Card>
          </Col>)}
      ...
      <Row gutter={16}>
        <Col xs={24} md={14}>
          <Card title="Recent Orders" style={{
          borderRadius: 12
        }}>
            <Table dataSource={(data?.recentOrders || []).map((o, i) => ({
            ...o,
            key: i
        ...
        <Col xs={24} md={10}>
          <Card title="Low Stock Alerts" style={{
          borderRadius: 12
        }} extra={<Tag color="red">{(data?.lowStockProducts || []).length} items</Tag>}>
            <Table dataSource={(data?.lowStockProducts || []).map((p, i) => ({
            ...p,
            key: i
          }))} pagination={false} size="small" columns={[{
            title: 'Product',
