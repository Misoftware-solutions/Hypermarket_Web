...
import { Table, Tag, Button, Typography, Card, Input, Select, Space, Row, Col, Statistic, Badge, InputNumber, Modal, Tooltip, message } from 'antd';
import { SearchOutlined, WarningOutlined, SyncOutlined, QuestionCircleOutlined } from '@ant-design/icons';
...
const AdminInventory = () => {
  ...
  const columns = [{
    ...
    render: (name, r) => <div><Text strong>{name}</Text><br /><Text type="secondary" style={{
        fontSize: '0.8rem'
      }}>{r.category_name}</Text></div>
  }, {
    title: 'Available',
    dataIndex: 'available_qty',
    render: (v, r) => {
      if (r.status === 'critical') return <Badge status="error" text={<Text type="danger" strong>{v}</Text>} />;
      if (r.status === 'low') return <Badge status="warning" text={<Text style={{
        color: '#faad14'
      }} strong>{v}</Text>} />;
    ...
    title: 'Actions',
    render: (_, r) => <Button type="primary" size="small" icon={<SyncOutlined />} onClick={() => {
      setUpdateModal({
        open: true,
        id: r.product_id,
        name: r.product_name,
  ...
  return <div>
      ...
      padding: 0
        ...
        <Table columns={columns} dataSource={items.map(it => ({
        ...it,
        key: it.product_id
      ...
      <Modal title={`Update Stock — ${updateModal.name}`} open={updateModal.open} onCancel={() => setUpdateModal({
      open: false,
      id: 0,
      name: '',
      stock: 0
