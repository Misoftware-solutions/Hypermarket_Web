...
import { Steps, Card, Radio, Button, Typography, Divider, Form, Input, Select, Row, Col, Tag, Space, message } from 'antd';
...
const Checkout = () => {
  ...
  const addresses = [
    { id: 1, label: 'Home', address: '123, MG Road, Koramangala', city: 'Bangalore', state: 'Karnataka', pincode: '560034', isDefault: true },
    { id: 2, label: 'Work', address: '456, Brigade Road, Indiranagar', city: 'Bangalore', state: 'Karnataka', pincode: '560038', isDefault: false },
  ];
  ...
  if (orderPlaced) {
    return (
      <div className="d-flex flex-column align-items-center justify-content-center" style={{ minHeight: 'calc(100vh - 64px)' }}>
        ...
        <Text type="secondary" style={{ fontSize: '1.1rem' }}>Your order #ORD-1006 has been confirmed</Text>
        <Text type="secondary" className="mb-4">Estimated delivery: 2-3 hours</Text>
        <Space size="large">
      ...
      content: (
        <div>
              ...
              {addresses.map(addr => (
                <Col xs={24} md={12} key={addr.id}>
                  <Radio value={addr.id} className="w-100">
                    <Card size="small" style={{ borderRadius: 10, border: selectedAddress === addr.id ? '2px solid #1890ff' : undefined, cursor: 'pointer' }}>
          ...
          <Form layout="vertical" size="large">
            <Row gutter={16}>
              <Col xs={24} md={8}><Form.Item label="Label"><Select options={[{ value: 'Home', label: 'Home' }, { value: 'Work', label: 'Work' }, { value: 'Other', label: 'Other' }]} /></Form.Item></Col>
    ...
    },
    {
      ...
      content: (
        <div>
          ...
          <Radio.Group defaultValue="slot1" className="w-100">
            <Row gutter={[16, 16]}>
              {[
                { key: 'slot1', label: 'Express (1-2 hrs)', desc: 'Fastest delivery', extra: '₹30' },
                { key: 'slot2', label: 'Today (4-6 hrs)', desc: 'Same day delivery', extra: 'FREE' },
                { key: 'slot3', label: 'Tomorrow (9 AM - 12 PM)', desc: 'Morning slot', extra: 'FREE' },
                { key: 'slot4', label: 'Tomorrow (2 PM - 5 PM)', desc: 'Afternoon slot', extra: 'FREE' },
              ].map(slot => (
        ...
        <div>
          ...
          <Radio.Group value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-100">
            <Space direction="vertical" className="w-100" size="middle">
              {[
                { key: 'online', label: 'Online Payment', desc: 'Pay via Razorpay (UPI, Cards, Net Banking)' },
                { key: 'cod', label: 'Cash on Delivery', desc: 'Pay when your order is delivered' },
                { key: 'wallet', label: 'Wallet Balance', desc: 'Available: ₹250.00' },
              ].map(pm => (
                <Radio value={pm.key} key={pm.key} className="w-100">
                  <Card size="small" style={{ borderRadius: 10, width: '100%', border: paymentMethod === pm.key ? '2px solid #1890ff' : undefined }}>
      ...
      <Row gutter={24}>
        <Col xs={24} md={16}>
          <Card style={{ borderRadius: 12 }}>
            <Steps current={currentStep} items={steps.map(s => ({ title: s.title, icon: s.icon }))} className="mb-4" />
            ...
            <div className="d-flex justify-content-between">
              ...
              {currentStep < steps.length - 1 ? (
                <Button type="primary" onClick={() => setCurrentStep(c => c + 1)}>Next</Button>
              ) : (
                <Button type="primary" size="large" onClick={handlePlaceOrder} style={{ height: 48, paddingInline: 40 }}>Place Order — ₹{total}</Button>
              )}
            </div>
          </Card>
        </Col>
        <Col xs={24} md={8}>
          <Card title="Order Summary" style={{ borderRadius: 12, position: 'sticky', top: 80 }}>
            {cartItems.map((item, i) => (
              <div key={i} className="d-flex justify-content-between mb-2"><Text>{item.name} × {item.qty}</Text><Text>₹{item.price * item.qty}</Text></div>
            ))}
            <Divider />
            ...
            <Divider />
            <div className="d-flex justify-content-between"><Title level={4} style={{ margin: 0 }}>Total</Title><Title level={4} style={{ margin: 0, color: '#1890ff' }}>₹{total}</Title></div>
          </Card>
        </Col>
      </Row>
    </div>
